...
另一方面，Saks、Srinivasan、Zhou 和 Zuckerman \cite{SSZZ00_connection_CR_min_wise} 首先将最小哈希的构造归约为组合矩形（combinatorial rectangle）的伪随机生成器问题，并基于该归约给出了种子长度为 $O(\log^{3/2} NM)$ 的构造。此后，针对组合矩形的伪随机生成器经历了长期的发展与改进，目前最好的结果是 Gopalan 与 Yehudayoff \cite{GY20_PRG_CR_SOTA} 的构造，这给出了 $O(\log NM \cdot \log \log NM)$ 的最小哈希构造。
...
对于两个哈希函数族 $\mathcal{H}, \mathcal{G}$，定义它们的\emph{直和}（direct sum）为一\emph{可重集}
$$
