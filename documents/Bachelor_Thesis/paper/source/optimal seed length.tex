...
\begin{restatable}{lemma}{ExtractorUniform}\label{lem:extractor_unif}
    给定任意的 $n$，熵 $k \le n$ 与 $m \le k / 2$，和任意误差 $\varepsilon$，均存在一个显式的 $(k, \varepsilon)$-提取器 $\Ext : \{0, 1\}^n \times \{0, 1\}^d \to \{0, 1\}^m$，满足 $d = O(\log (n / \varepsilon))$。此外，$\Ext$ 满足一个额外性质：对于任意固定种子 $s \in \{0, 1\}^d$，均有 $\Ext(U_n, s) = U_m$。 
\end{restatable}
...
本节将详细阐述构造显式最小哈希族的核心技术路线。我们的构造可将种子长度压缩至最优的 $O(\log N)$，同时保持亚常数级的乘法误差 $\delta = 2^{-O\left(\frac{\log N}{\log \log N}\right)}$。
...
对于误差分析，我们首先关注的还是分组函数 $g$ 的性质。我们设 $J := \{g(y) : y \in Y\}$，$B_i := \{x \in X : g(x) = i\},\ \forall i \in [\ell]$，而 $B_J := \bigcup_{j \in J} B_j$。下面引理的证明与 \cref{lem:allocation_small_error} 高度相似，我们省略细节的论证。
...
\end{align*}
其中有关 $J$ 的项不写明 $\sigma_j$ 而是直接用记号 $h$ 来表示，这是因为与 $J$ 相关的桶中同时包含 $Y$ 中元素和 $X$ 中元素，其函数行为由 $\varphi+\psi$ 共同决定。严格地说，可以先枚举 $Y$ 中取得最大哈希值的非空子集 $Y'$，并分别规定与 $Y'$ 以及 $Y \setminus Y'$ 有关桶的函数要求。由于这一表述会引入较多额外记号，以下仍采用上述简化写法。

对于不包含 $Y$ 中元素的桶，即 $i\notin J$，我们可以暂时忽略外加的 $(C_e+1)k$ 次独立函数 $\psi$。原因在于：固定 $\psi$ 后，$\psi$ 在每个桶中只是对哈希值进行了一个固定平移，而组合矩形函数族对于这种平移保持封闭。因此，关于种子 $s_i$ 的事件仍然可以直接使用 $\PRG_1$ 的伪随机性质进行分析。等价地，我们可以将 $\psi$ 吸收到阈值中，并继续分析 $\sigma_i(B_i)$。而对于与 $Y$ 有关的桶，由于需要精确恢复 $Y$ 中元素之间的联合顺序关系，不能简单忽略 $\psi$，这也是后续需要额外添加 $\psi$ 的根本原因。
...
最后求和汇总，误差项的分析与上一节完全类似；主项的分析也可以通过相同的分段求和方法完成。由于其计算过程与上一节高度重复，且不会引入新的技术困难，此处省略具体展开。这样我们便完成了证明。
