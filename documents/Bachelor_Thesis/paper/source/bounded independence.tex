...
$t$ 次独立性实际上也是一种伪随机生成器。具体地，它是所有 \emph{$t$-集团函数}的伪随机生成器，并且误差为 $0$。这里的 $t$-集团函数是指所有依赖于至多 $t$ 个输入位置的布尔函数。
...
本节证明，为了仅依靠有限次独立性得到误差为 $\delta$ 的 $k$-最小哈希族，所需独立次数至少为
$$
    \Omega\left(k+\log\frac{1}{\delta}\right).
$$

首先证明 $\Omega(k)$ 下界。任取不相交的集合 $X,Y$，其中 $|X|=s$、$|Y|=k$，这里 $s$ 可以是任意正整数。先令哈希值取自 $[0,1)$。$X$ 中所有元素的哈希值相互独立且均匀分布。对于 $Y$，先随机生成其中 $k-1$ 个元素的哈希值，再生成最后一个元素，使得 $Y$ 中落入上半区 $[1/2,1)$ 的元素个数为偶数；在确定其落入上半区或下半区后，再在相应半区内均匀取值。$X$ 与 $Y$ 的哈希值相互独立。

该分布是精确的 $(k-1)$ 次独立分布，因为偶数约束只决定全部 $k$ 个元素之间的联合关系，任意至多 $k-1$ 个元素仍然相互独立且均匀分布。这种通过固定上下半区落点个数奇偶性的构造方式，在 Pătraşcu 和 Thorup~\cite{PT_lb_t_wise_min_wise} 对最小哈希族下界的证明中也有用到。

考虑事件 $E=\{\max h(Y)<\min h(X)\}$，并记 $\mathcal P$ 为“$Y$ 中落入上半区的元素个数为偶数”这一事件。上述分布正是完全随机分布条件于 $\mathcal P$ 后得到的，并且 $\Pr[\mathcal P]=1/2$。在完全随机哈希下，
$$
    \Pr[E]=\binom{s+k}{k}^{-1}.
$$

令 $n=s+k$，并令 $Z$ 表示 $X\cup Y$ 中落入下半区 $[0,1/2)$ 的元素个数。完全随机哈希可以等价地按如下两步生成：先独立均匀地抽取 $n$ 个数并将其从小到大排列，再将 $X\cup Y$ 中的 $n$ 个元素均匀随机地分配到这 $n$ 个位置上。由于所有元素的哈希值独立同分布，后一随机排列与前一步抽取到的有序数值相互独立。

事件 $E$ 只要求最小的 $k$ 个位置恰好由 $Y$ 中元素占据，而 $Z$ 只取决于这 $n$ 个有序数值中有多少个小于 $1/2$。因此，条件于 $E$ 不会改变 $Z$ 的分布，即
$$
    Z\mid E\sim\operatorname{Bin}\left(n,\frac12\right).
$$

在事件 $E$ 下，若 $Z=j<k$，则 $Y$ 中恰有 $j$ 个元素落入下半区，因而有 $k-j$ 个元素落入上半区；若 $Z\ge k$，则 $Y$ 中所有元素均落入下半区。因此
\begin{align*}
    \Pr[\mathcal P\mid E]
    &=
    \frac{1}{2^n}
    \left(
        \sum_{j = k - 2, k - 4, \ldots}
        \binom{n}{j}
        +
    ...
    \frac12\left(
        1+
        \frac{1}{2^{n-1}}
        \sum_{j=k}^{n-1}\binom{n-1}{j}
    \right)\\
    &=
    \frac12\left(
        1+
        \Pr\left[
            \operatorname{Bin}\left(n-1,\frac12\right)\ge k
        \right]
    \right).
\end{align*}

由于上述 $(k-1)$ 次独立分布正是完全随机分布条件于 $\mathcal P$ 后得到的，并且 $\Pr[\mathcal P]=1/2$，由贝叶斯公式可得
\begin{align*}
    \Pr[E\mid\mathcal P]
    &=
    \frac{\Pr[E]\Pr[\mathcal P\mid E]}
         {\Pr[\mathcal P]}\\
    &=
    \left(
        1+
        \Pr\left[
            \operatorname{Bin}\left(s+k-1,\frac12\right)\ge k
        \right]
    \right)
    \binom{s+k}{k}^{-1}.
\end{align*}

因此，该 $(k-1)$ 次独立分布产生的乘法误差恰好为
$$
    \varepsilon_{k,s}
    =
    \Pr\left[
    ...
    \right].
$$
该公式对任意 $s$ 都成立。当 $s\ge k$ 时，$\varepsilon_{k,s}\ge 1/2$；而当 $s\to\infty$ 时，$\varepsilon_{k,s}\to1$。因此，对于任意 $\delta<1$，总可以选取足够大的 $X$，使得 $\varepsilon_{k,s}>\delta$。这说明 $(k-1)$ 次独立性不足以保证误差为 $\delta$ 的 $k$-最小哈希性质，从而所需独立次数至少为 $\Omega(k)$。

上述构造也可以直接离散化到充分大的偶数值域 $[M]$：先决定元素落入值域的上半区或下半区，再在相应半区内均匀取值。随着 $M$ 增大，严格次序事件的概率任意接近上述连续情形，因此下界保持成立。

另一方面，Pătraşcu 和 Thorup \cite{PT_lb_t_wise_min_wise} 证明了普通最小哈希在有限次独立性下的如下下界。

\begin{theorem}[\cite{PT_lb_t_wise_min_wise} 中的定理 3.1]\label{thm:PT_lb_t_wise_min_wise}
    对于任意大小为 $n$ 的键集 $X$ 和另一个键 $y$，存在一个 $t$ 次独立哈希族
    $$
        \mathcal H=\{h:X\cup\{y\}\to[0,1)\},
    $$
    使得
    $$
        \Pr_{h\sim\mathcal H}[h(y)<\min h(X)]
        =
        \frac{1+2^{-O(t)}}{n+1}.
    $$
\end{theorem}

由于 $k$-最小哈希的定义包含 $|Y|=1$ 的普通最小哈希情形，为了将乘法误差控制在 $\delta$ 以内，必须有
$$
    t=\Omega\left(\log\frac{1}{\delta}\right).
$$
结合上述 $\Omega(k)$ 下界可得
$$
    t
    ...
    \right).
$$
具体而言，当 $\delta<2^{-k}$ 时，$\log 1/\delta$ 项主导；当 $\delta\ge2^{-k}$ 时，$k$ 项主导。
